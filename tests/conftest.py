...
def setup(select_browser):
    login_page = LoginPages(browser=select_browser)
    yield login_page
    login_page.close()

@pytest.fixture
def run_all_browser(all_browser):
    login_page = LoginPages(browser=all_browser)
    yield login_page
    login_page.close()
...
def has_product_in_cart(login_saucedemo):
    ...
    assert product_p.get_cart_badge_number() == 1, 'Quantidade de produtos no carrinho está incorreto'
